...
<?php $__env->startSection('content'); ?>
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <?php echo Former::select('project_id')->addOption('', '')->addGroupClass('project-select')
                    ->label(trans('texts.project')); ?>

            <?php echo Former::textarea('description')->rows(3); ?>
                ...
                <div class="form-group simple-time" id="editDetailsLink">
                    ...
                    <div class="col-lg-8 col-sm-8" style="padding-top: 10px">
                        ...
                        <?php if(!$task->is_running): ?>
                            <p><?php echo Button::primary(trans('texts.edit_details'))->withAttributes(['onclick'=>'showTimeDetails()'])->small(); ?></p>
                        <?php endif; ?>
                    </div>
                </div>
                            ...
                            <td style="padding: 0px 12px 12px 0 !important">
                                <div data-bind="css: { 'has-error': !isStartValid() }">
                                    <input type="text" data-bind="dateTimePicker: startTime.pretty, event:{ change: $root.refresh }"
                                        class="form-control time-input" placeholder="<?php echo e(trans('texts.start_time')); ?>"/>
                                </div>
                            </td>
                            <td style="padding: 0px 12px 12px 0 !important">
                                <div data-bind="css: { 'has-error': !isEndValid() }">
                                    <input type="text" data-bind="dateTimePicker: endTime.pretty, event:{ change: $root.refresh }"
                                        class="form-control time-input" placeholder="<?php echo e(trans('texts.end_time')); ?>"/>
                                </div>
                            </td>
    ...
    <?php if(Auth::user()->canCreateOrEdit(ENTITY_TASK, $task)): ?>
        <?php if(Auth::user()->hasFeature(FEATURE_TASKS)): ?>
            ...
            <?php elseif($task && $task->is_deleted): ?>
                <?php echo Button::normal(trans('texts.cancel'))->large()->asLinkTo(URL::to('/tasks'))->appendIcon(Icon::create('remove-circle')); ?>
    ...
    var timeLabels = {};
    <?php foreach(['hour', 'minute', 'second'] as $period): ?>
        timeLabels['<?php echo e($period); ?>'] = '<?php echo e(trans("texts.{$period}")); ?>';
        timeLabels['<?php echo e($period); ?>s'] = '<?php echo e(trans("texts.{$period}s")); ?>';
    <?php endforeach; ?>
